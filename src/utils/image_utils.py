...
_adaptive_loader = AdaptiveImageLoader()


def http_get(url, timeout_seconds=30.0, headers=None):
    return requests.get(url, timeout=timeout_seconds, headers=headers)
...
def get_image(image_url, timeout_seconds=30.0):
    response = http_get(image_url, timeout_seconds=timeout_seconds)
    img = None
