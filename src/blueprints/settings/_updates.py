    ...
    try:
        ...
        if _mod._systemd_available():
            ...
            try:
                ...
                _mod.logger.exception("systemd-run failed; falling back to thread runner")
...
def update_status():
    try:
        ...
        if running:
            ...
            if not cleared and started_at and (time.time() - float(started_at)) > _mod._UPDATE_TIMEOUT_SECONDS:
                _mod._UPDATE_STATE["last_unit"] = unit
                _mod._set_update_state(False, None)
