...
def parse_forecast(daily_forecast, tz, current_suffix, lat, plugin_dir):
    ...
    for day in daily_forecast:
        ...
        moon_icon_path = get_moon_phase_icon_path(phase_name_north_hemi, lat, plugin_dir)
...
def parse_data_points(weather, air_quality, tz, units, time_format, plugin_dir):
    ...
    if sunrise_epoch:
        ...
        data_points.append({
            "label": "Sunrise",
            "measurement": format_time(sunrise_dt, time_format, include_am_pm=False),
...
def parse_open_meteo_data_points(weather_data, aqi_data, tz, units, time_format, plugin_dir):
    ...
    data_points.append({
        ...
        "icon": os.path.join(plugin_dir, 'icons/wind.png'), "arrow": wind_arrow
    })
    ...
    data_points.append({
        ...
        "unit": scale, "icon": os.path.join(plugin_dir, 'icons/aqi.png')
    })
...
def parse_weather_data(weather_data, aqi_data, tz, units, time_format, lat, plugin_dir):
    ...
    data['forecast'] = parse_forecast(weather_data.get('daily'), tz, current_suffix, lat, plugin_dir)
    data['data_points'] = parse_data_points(weather_data, aqi_data, tz, units, time_format, plugin_dir)

    data['hourly_forecast'] = parse_hourly(weather_data.get('hourly'), tz, time_format, units)
    return data


def parse_open_meteo_data(weather_data, aqi_data, tz, units, time_format, lat, plugin_dir):
    ...
    data['forecast'] = parse_open_meteo_forecast(weather_data.get('daily', {}), tz, is_day, lat, plugin_dir)
    data['data_points'] = parse_open_meteo_data_points(weather_data, aqi_data, tz, units, time_format, plugin_dir)

    data['hourly_forecast'] = parse_open_meteo_hourly(weather_data.get('hourly', {}), tz, time_format)
    return data
