...
def test_parse_open_meteo_forecast_handles_api_and_fallback(monkeypatch, weather_plugin):
	w = weather_plugin
	...
	class DummyResp:
		...
		def json(self):
			return self._json
	monkeypatch.setattr('requests.get', lambda *args, **kwargs: DummyResp([{"Phase": "Full Moon", "Illumination": 0.5}]))
...
def test_parse_data_points_and_open_meteo_points(weather_plugin):
	...
	points = w.parse_data_points(weather, air_quality, pytz.timezone('UTC'), 'metric', '24h')
	labels = [p['label'] for p in points]
	assert 'Sunrise' in labels and 'Sunset' in labels and 'Air Quality' in labels

	# Open-Meteo style
	weather_data = {
		"current_weather": {"time": '2020-01-01T00:00', "temperature": 5, "apparent_temperature": 4, "windspeed": 3},
		"daily": {"sunrise": ['2020-01-01T07:00'], "sunset": ['2020-01-01T17:00']},
		"hourly": {"time": ['2020-01-01T00:00'], "relative_humidity_2m": [50], "surface_pressure": [1015], "visibility": [10000]}
	}
	aqi_data = {"hourly": {"time": ['2020-01-01T00:00'], "uv_index": [1], "european_aqi": [10]}}
	points2 = w.parse_open_meteo_data_points(weather_data, aqi_data, pytz.timezone('UTC'), 'metric', '24h')
