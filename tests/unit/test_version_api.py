...
def test_api_version_returns_current(client):
    ...
    with patch("blueprints.settings._requests.get", return_value=_mock_github_response()):
...
def test_api_version_cache_miss_fetches_from_github(flask_app, client):
    ...
    with patch("blueprints.settings._requests.get", return_value=_mock_github_response("v2.0.0")):
        resp = client.get("/api/version")
...
def test_api_version_offline_returns_null_latest(client):
    ...
    with patch("blueprints.settings._requests.get", side_effect=Exception("network error")):
        resp = client.get("/api/version")
...
def test_github_api_404(client):
    ...
    with patch("blueprints.settings._requests.get", return_value=_mock_github_response(status_code=404)):
        resp = client.get("/api/version")
...
def test_github_repo_env_override(client, monkeypatch):
    ...
    with patch("blueprints.settings._requests.get", return_value=_mock_github_response()) as mock_get:
