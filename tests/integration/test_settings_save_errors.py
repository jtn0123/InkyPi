...
def test_save_settings_invalid_unit(client):
    data = {
        'deviceName': 'D', 'orientation': 'horizontal', 'invertImage': '', 'logSystemStats': '',
        'timezoneName': 'UTC', 'timeFormat': '24h', 'interval': '1', 'unit': 'dayz',
...
def test_save_settings_invalid_interval_and_bounds(client):
    ...
    data = {
        'deviceName': 'D', 'orientation': 'horizontal', 'invertImage': '', 'logSystemStats': '',
        'timezoneName': 'UTC', 'timeFormat': '24h', 'interval': 'x', 'unit': 'minute',
        'saturation': '1.0', 'brightness': '1.0', 'sharpness': '1.0', 'contrast': '1.0'
    }
    resp = client.post('/save_settings', data=data)
    assert resp.status_code == 400
