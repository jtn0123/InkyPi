...
def test_display_plugin_instance_returns_500_on_plugin_error(client, device_config_dev, monkeypatch):
...
def test_add_plugin_returns_500_when_write_config_fails(client, device_config_dev, monkeypatch):
    # Existing playlist
    pm = device_config_dev.get_playlist_manager()
    pm.add_playlist("P1", "00:00", "24:00")
    ...
    payload = {
        'plugin_id': 'ai_text',
        'refresh_settings': json.dumps({'playlist': 'P1', 'instance_name': 'inst', 'refreshType': 'interval', 'unit': 'minute', 'interval': 1}),
        **_valid_ai_text_settings(),
    }
    os.environ['OPEN_AI_SECRET'] = 'test'
