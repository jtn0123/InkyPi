...
- GitHub Dependabot reports 2 open moderate alerts for `pip` (`GHSA-58qw-9mgm-455v` / `CVE-2026-3219`), one in `uv.lock` and one in `install/requirements-dev.txt`; GitHub currently lists no patched version.
...
**Top 5 highest-leverage fixes:** A1, C1, D1, I1, F1
...
| F | Dependencies & Tech Currency | B- | B | B- | Down: uv runtime locking is strong, but the live Dependabot view now has 2 open medium `pip` alerts with no patched version. |
...
Runtime dependency handling is mostly solid: `pyproject.toml` declares Python `>=3.11,<3.14`, `uv.lock` is present, docs explain dependency locking, and CI runs lockfile drift plus security/SBOM checks. The grade drops to B- because live Dependabot currently reports 2 open moderate `pip` alerts (`uv.lock` and `install/requirements-dev.txt`) and dev dependencies are still handled through a separate pip-compile style workflow. The `pip` advisory currently has no patched version, so this is partly a tracking/process issue rather than a straightforward bump.

#### F1 - Track and close the open `pip` advisory
- **Where:** `uv.lock`, `install/requirements-dev.txt`, GitHub Dependabot alerts #7 and #8
- **What's wrong:** GitHub reports `GHSA-58qw-9mgm-455v` / `CVE-2026-3219` for `pip` in both runtime lock and dev requirements metadata. The alert has no patched version yet, so it cannot be resolved by a normal version bump today.
- **Fix:** Add a short tracking issue or PR note that records the advisory, affected manifests, current lack of patched version, and planned closure condition. Once a patched `pip` release exists, refresh `uv.lock` and regenerated dev requirements, then verify Dependabot closes both alerts.
- **Effort:** S
- **Grade lift:** B- -> B (turns a live security/dependency finding into an owned closure path)
...
4. **F1:** keep the new `pip` advisory visible until a patched release exists, then close both Dependabot alerts.
